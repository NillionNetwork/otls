...
public:
    g2_t h;    ///< Secondary generator point for the commitment
     ...
     * 
     * @param h The secondary generator point (g2_t) to use for the commitment.
     */
    RelicPedersenComm(const g2_t h) {
     ...
     * 
     * @param[out] com The resulting commitment point (g2_t).
     * @param[out] rnd The random blinding factor used (bn_t).
     * @param[in]  msg The message to commit to (bn_t).
     */
    inline void commit(g2_t com, bn_t rnd, const bn_t msg) {
        // Initialize
        g2_t msg_G;
        g2_null(msg_G);
        g2_new(msg_G);
        ...
        g2_mul(com, h, rnd);        // com = rnd*h
        g2_mul_gen(msg_G, msg);     // msg_G = msg*G
        g2_add(com, msg_G, com);    // com = msg_G + com
     ...
     * 
     * @param[in] com The commitment point to verify (g2_t).
     ...
     */
    inline bool open_check(const g2_t com, const bn_t rnd, const bn_t msg) {
        // Initialize
        g2_t expected_com;
        g2_t msg_G;
        g2_null(expected_com);
        g2_null(msg_G);
        g2_new(expected_com);
        g2_new(msg_G);
        
        // Compute expected commitment
        g2_mul(expected_com, h, rnd);      // expected_com = rnd*h
        g2_mul_gen(msg_G, msg);            // msg_G = msg*G
        g2_add(expected_com, msg_G, expected_com); // expected_com = msg_G + expected_com
        
        // Compare points
        bool res = (g2_cmp(com, expected_com) == RLC_EQ);
        
        // Clean up
        g2_free(expected_com);
        g2_free(msg_G);
        return res;
    }
     ...
     * 
     * @param res   Output: result as a RELIC g2_t (must be initialized)
     * @param coms  Array of RELIC g2_t commitment points
     * @param coef  Array of RELIC bn_t coefficients
     * @param len   Number of elements in the arrays
     */
    inline void linear_comb_com(g2_t res, const g2_t* coms, const bn_t* coef, size_t len) {
        g2_t tmp;
        g2_null(tmp);
        g2_new(tmp);
...
class ComConv {
   public:
    ...
    bool compute_com_send(g2_t* com,
                          vector<block> bKEYs,
        ...
        // }
        int compressed_size = g2_size_bin(com[0], 1);  // 1 for compressed format
        ...
        for (size_t i = 0; i < chunk_len; i++) {
            ...
            g2_read_bin(com[i], buf, compressed_size);
        }
        ...
        io->recv_data(buf, compressed_size);
        chi_hash.put(buf, compressed_size);
        g2_read_bin(relic_comm_r, buf, compressed_size);
        delete[] buf;
        ...
        g2_t relic_comms[chunk_len + 1];
        // Initialize all points
        for (size_t i = 0; i < chunk_len; i++) {
            g2_null(relic_comms[i]);
            g2_new(relic_comms[i]);
            g2_copy(relic_comms[i], com[i]);  // Copy existing commitments
        }
        // Initialize and copy the last point (relic_comm_r)
        g2_null(relic_comms[chunk_len]);
        g2_new(relic_comms[chunk_len]);
        g2_copy(relic_comms[chunk_len], relic_comm_r);
        ...
        //     g2_free(relic_comms[i]);
        // }

        return res;
    }

    std::tuple<bool, vector<BIGNUM*>, vector<unsigned char*>, vector<int>> compute_com_recv(g2_t* com,
                          vector<BIGNUM*>& rnds,
                          vector<block> bMACs,
                          RelicPedersenComm& pc,
                          uint64_t batch_size) {
        ...
        //     chi_hash.put(buf, 65);
        // }
        int compressed_size = g2_size_bin(relic_comm_r, 1);  // 1 for compressed format
        // unsigned char* buf = new unsigned char[compressed_size];  
        unsigned char* buf = (unsigned char*)malloc(compressed_size); 
        for (size_t i = 0; i < chunk_len; i++) {
            g2_write_bin(buf, compressed_size, com[i], 1);  // 1 for compressed format
        ...
        // io->send_data(buf, 65);
        // chi_hash.put(buf, 65);
        g2_write_bin(buf, compressed_size, relic_comm_r, 1);
        io->send_data(buf, compressed_size);
        chi_hash.put(buf, compressed_size);
    ...
    bool sign_commitments_with_ecdsa(
        ...
        for (size_t i = 0; i < chunk_len; i++) {
            ...
            int compressed_size = g2_size_bin(com[0], 1);
            unsigned char point_buffer[compressed_size];
            g2_write_bin(point_buffer, compressed_size, com[i], 1);
