...
class ComConv {
   public:
    ...
    bool sign_commitments_with_ecdsa(
        ...
        // 2. Sign all commitment in the same hash stream
        ...
        for (size_t i = 0; i < chunk_len; i++) {
            ...
            // Feed the compressed point into the hash stream
        ...
        if (!signature) {
            fprintf(stderr, "Error: ECDSA signing failed for the commitment\n");
        ...
        if (sig_len <= 0) {
            fprintf(stderr, "Error: Failed to convert signature to DER format for the commitment\n");
    ...
    std::tuple<bool, vector<unsigned char*>, vector<int>> receive_commitment_signature(
        ...
        if (num_signatures != 1) {
            fprintf(stderr, "Error: Number of signatures was expected to be 1, but was %d\n", 
                    num_signatures);
        ...
        if (!sig_der) {
            fprintf(stderr, "Error: Memory allocation failed for the signature\n");
            if (verify_key) EC_KEY_free(verify_key);
        ...
        // Build hash of all commitments to verify the signature
        ...
        if (verify) {
            ...
            if (!signature) {
                fprintf(stderr, "Error: Failed to parse signature for the commitment\n");
                EC_KEY_free(verify_key);
                return std::make_tuple(false, vector<unsigned char*>(), vector<int>());
            }
            ...
            if (verify_result != 1) {
                fprintf(stderr, "Error: Signature verification failed for the commitment\n");
                EC_KEY_free(verify_key);
                return std::make_tuple(false, vector<unsigned char*>(), vector<int>());
            } else {
                signatures_valid = true;
